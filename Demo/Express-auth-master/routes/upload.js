        ...
        if (!isExist) {
            fs.mkdirSync(tempPath, (err) => {
                if (err) {
                    console.log(err)
                    uploadStatus = false
                    uploadMessage = "Path resolution failed, please contact administrator"
...
var fileFilter = (req, file, callback) => {
    ...
    if (!fileTypeList) {
        uploadStatus = false
        uploadMessage = "Upload failed. Incorrect upload type"
        return callback(null);
    }
    var fileType = file.originalname.substr(file.originalname.lastIndexOf("."));
    // 判断是否格式出错
    if (fileTypeList.indexOf(fileType.toLowerCase()) == -1) {
        uploadStatus = false
        uploadMessage = "Upload failed, picture format error"
        return callback(null);
    }
    callback(null, file)
}
...
router.post("/uploadFile", upload.single("file"), (req, res, next) => {
    ...
    if (!req.file) {
        return res.send({
            code: 201,
            message: "Upload failed. Upload parameter type is wrong",
            data: null
        })
    }
    if (req.file.size > size) {
        ...
        return res.send({
            code: 201,
            message: "Upload failed. File size must not exceed" + size / 1024 + "K",
            data: null
        })
    }
    res.send({
...
router.post("/editor-upload", (req, res, next) => {
    ...
    form.parse(req, function (err, fields, files) {
        ...
        if (!uploadfoldername) {
            return res.send({ errno: 201, message: "User information missing" })
        }
        ...
        if (!isExist) {
            fs.mkdirSync(uploadfolderpath, (err) => {
                if (err) {
                    console.log(err)
                    return res.send({ errno: 201, message: "Path resolution error" })
                }
            })
        }
        ...
        //         console.log(err)
        //         res.send({ errno: 201, message: "上传失败，文件转换失败" })
        //     } else {
        //         // 保存成功
        //         console.log('fs.rename done');
        //         // 拼接图片url地址
        //         imgUrl = assitUrl + "/user-" + uploadfoldername + '/' + filename;
        //         res.send({ errno: 0, message: "上传成功", data: [imgUrl] })
        //     }
        // });
        ...
        readStream.on('end', function () {
            ...
            imgUrl = assitUrl + "/user-" + uploadfoldername + '/' + filename;
            res.send({ errno: 0, message: "successfully upload", data: [imgUrl] })
        });
